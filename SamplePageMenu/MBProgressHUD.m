...
+ (MB_INSTANCETYPE)showTransitionToParentView:(UIView *)view withImage:(UIImage*)aImage withBGColor:(UIColor*)aColor
{
    MBProgressHUD *hud = [[self alloc] initWithView:view];
    UIImageView *imageDefault = [[UIImageView alloc] initWithFrame:view.bounds];
    imageDefault.userInteractionEnabled = YES;
    imageDefault.contentMode = UIViewContentModeScaleAspectFit;
    imageDefault.image = aImage;
    imageDefault.backgroundColor = aColor;
    imageDefault.frame = view.bounds;
    [view addSubview:imageDefault];
    imageDefault.tag = 798080;
    return MB_AUTORELEASE(hud);
}

+ (BOOL)hideTransitionView:(UIView *)inProgressView1
{
    UIImageView *imgaview = (UIImageView*)[inProgressView1 viewWithTag:798080];
    [imgaview removeFromSuperview];
    imgaview = nil;
    
    MBProgressHUD *hud = [self HUDForView:inProgressView1];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
        return YES;
    }
